...
RuneTab:AddButton({
    ...
    Callback = function()
        ...
        if currentPrisms and chromatizePrice then
            local canAfford = currentPrisms >= chromatizePrice
                    ...
                    if chromatizeButton then
                        ...
                        if billboardGui then
                            ...
                            if priceLabel and priceLabel.Text then
                                ...
                                local patterns = {
                                    "([%d%.e%-+]+)%s*PRISMS",  -- "5e178 PRISMS"
                                    "([%d%.e%-+]+)%s*Prisms",  -- "5e178 Prisms"
                                    "([%d%.e%-+]+)",           -- Just the number
                                    "Price:%s*([%d%.e%-+]+)",  -- "Price: 5e178"
                                    "Cost:%s*([%d%.e%-+]+)"    -- "Cost: 5e178"
                                }
                                
                                for _, pattern in ipairs(patterns) do
                                    local numberStr = text:match(pattern)
                                    if numberStr then
                                        print("[DEBUG] Extracted number string: '" .. tostring(numberStr) .. "'")
                                        -- Clean up the number string
                                        numberStr = numberStr:gsub("e%+", "e") -- Remove + from scientific notation
                                        numberStr = numberStr:gsub("%s+", "")   -- Remove spaces
                                        ...
                                        if price and price > 0 and price ~= math.huge then
                                            ...
                                            print("[DEBUG] Failed to convert to valid number: " .. tostring(numberStr))
                                        end
                                    end
                                end
...
local function getCurrentPrisms()
    local success, result = pcall(function()
        ...
        local stat = player.Stats:FindFirstChild("Prisms")
        if stat and stat.Value then
            ...
            -- Extract numeric value from display format (e.g., "1.60e512" -> 1.60e512)
            local numericStr = displayValue:match("([%d%.e%-]+)")
            if numericStr then
                local numericValue = tonumber(numericStr)
                if numericValue then
                    print("[DEBUG] Using numeric value for comparison: " .. tostring(numericValue))
                    return numericValue
                end
            end
            
            print("[DEBUG] Failed to extract numeric value from: " .. tostring(displayValue))
            return nil
        end
        return nil
    end)
            ...
            local success, err = pcall(function()
                ...
                if currentPrisms and chromatizePrice and 
                   ...
                   currentPrisms > 0 and chromatizePrice > 0 then
                    ...
                    if currentPrisms >= chromatizePrice then
