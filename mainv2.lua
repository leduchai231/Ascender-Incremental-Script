...
local Window = OrionLib:MakeWindow({
    Name = "Ascender Incremental Hub v" .. SCRIPT_VERSION,
    HidePremium = false,
    SaveConfig = true,
    ConfigFolder = "AscenderIncrementalConfig",
    IntroEnabled = true,
    IntroText = "Ascender Incremental Script v" .. SCRIPT_VERSION,
...
-- Store toggle references for GUI updates
...
local function loadConfig(configName)
    ...
    if success and result then
        ...
        -- Update GUI toggles to reflect loaded config
        task.spawn(function()
            task.wait(0.1) -- Small delay to ensure GUI is ready
            if toggleReferences.antiAfk then
                toggleReferences.antiAfk:Set(antiAfkEnabled)
            end
            if toggleReferences.autoUpgrade then
                toggleReferences.autoUpgrade:Set(autoUpgradeEnabled)
            end
            if toggleReferences.autoAllRune then
                toggleReferences.autoAllRune:Set(autoAllRuneEnabled)
            end
            if toggleReferences.autoLevelChromatize then
                toggleReferences.autoLevelChromatize:Set(autoLevelChromatizeEnabled)
            end
...
MainTab:AddButton({
    ...
    Callback = function()
        OrionLib:MakeNotification({
            Name = "Update Check",
            Content = "You are running the latest version v" .. SCRIPT_VERSION,
            Image = "rbxassetid://4483345998",
            Time = 3
        })
    end
...
toggleReferences.autoLoadConfig = ConfigTab:AddToggle({
    Name = "Auto Load Config on Execute",
    Default = autoLoadConfigEnabled,
    Callback = function(Value)
        autoLoadConfigEnabled = Value
        OrionLib:MakeNotification({
            Name = "Auto Load Config",
            Content = Value and "Auto load config enabled - will load last config on script start" or "Auto load config disabled",
            Image = "rbxassetid://4483345998",
            Time = 3
        })
    end
...
TalentTreeTab:AddTextbox({
    ...
    Callback = function(Value)
        ...
        if numValue and numValue >= 0.001 and numValue <= 5 then
            ...
            OrionLib:MakeNotification({
                Name = "Invalid Input",
                Content = "Please enter a number between 0.001 and 5",
                Image = "rbxassetid://4483345998",
                Time = 3
            })
        end
...
toggleReferences.autoUpgrade = TalentTreeTab:AddToggle({
    Name = "Auto Upgrade",
    Default = false,
    Callback = function(Value)
...
toggleReferences.autoAllRune = RuneTab:AddToggle({
...
toggleReferences.autoLevelChromatize = RuneTab:AddToggle({
...
toggleReferences.antiAfk = SettingsTab:AddToggle({
    Name = "Anti AFK",
    Default = true,
    Callback = function(Value)
    ...
    Callback = function()
        ...
        if not success then
            OrionLib:MakeNotification({
                ...
                Content = "Failed to hop servers: " .. tostring(err),
                Image = "rbxassetid://4483345998",
                Time = 3
            })
        end
...
if autoLoadConfigEnabled then
    task.spawn(function()
        task.wait(2) -- Wait for GUI to fully load
        local lastConfigFile = "AscenderIncrementalLastConfig.txt"
        if isfile(lastConfigFile) then
            ...
            if success and lastConfig and lastConfig ~= "" then
                ...
                OrionLib:MakeNotification({
                    Name = "Auto Load Config",
                    Content = "Automatically loaded config: " .. lastConfig,
                    Image = "rbxassetid://4483345998",
                    Time = 3
                })
            end
...
saveConfig = function(configName)
    ...
    if autoLoadConfigEnabled and configName then
        local success = pcall(function()
            writefile("AscenderIncrementalLastConfig.txt", configName)
        end)
        if not success then
            warn("Failed to save last config name")
        end
    end
end
